...
local PLUGINS_PATH = PathUtil.plugins_path
local PLUGINS_DATA_PATH = PathUtil.root_path .. "/data/plugins"
...
function PluginsUtil.clearOpenedPluginPaths()
  activity.application.set("plugin_enabledpaths",nil)
end
...
function setEnabled(packageName, state)
  activity.setSharedData("plugin_" .. packageName .. "_enabled", state)
  return PluginsUtil
end
PluginsUtil.setEnabled = setEnabled

function getEnabled(packageName)
  local state = activity.getSharedData("plugin_" .. packageName .. "_enabled")
  if state == nil then
...
function onPluginError(titleName,packageName,message,funcName)
  ...
  .setTitle("Plugin " .. titleName)
  .setMessage(message)
  .setPositiveButton(res.string.ok, nil)
  .setNegativeButton(res.string.no, nil)
  .show()
  pcall(function()
    io.open("/sdcard/XCLUA/crash/"..activity.getPackageName().."_"..packageName..".txt","a"):write(funcName..os.date(" %Y-%m-%d %H:%M:%S").."\n"..message.."\n\n"):close()
  end)
end
...
function loadPlugins()
  plugins = {}
  --已启用的插件列表
  enabledPluginPaths=activity.application.get("plugin_enabledpaths")
  --插件全局事件
  local pluginsEvents = {}
  ...
  if not(enabledPluginPaths) then
    ...
    activity.application.set("plugin_enabledpaths",enabledPluginPaths)
  end
